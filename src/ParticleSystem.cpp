...
void ParticleSystem::setParticleColor(ofColor c) {
	color = c;
}
...
ThrustForce::ThrustForce(const ofVec3f &d) {
	dir = d;
}

void ThrustForce::updateForce(Particle * particle) {
	particle->forces.x += dir.x;
	particle->forces.y += dir.y;
	particle->forces.z += dir.z;
}
